...
| サービス | 料金 | 備考 |
|---------|------|------|
| [OpenAI Plus/Pro/Team](https://openai.com/) | $20〜/月 | サブスク加入済みならCodex CLIがそのまま使えます |
| [OpenAI API](https://platform.openai.com/) | 従量課金（$5〜/月目安） | APIキーで利用。サブスクなしでもOK |
| [Anthropic API](https://console.anthropic.com/) | 従量課金（$5〜/月目安） | Claude Codeの頭脳。Claudeが応答を生成 |

> 💡 **OpenAI Plus/Pro/Teamサブスク**に加入済みなら、APIキーなしでCodex CLIが使えます。  
> 初回起動時に `codex` コマンドでログインしてください。
...
A: **OpenAI Plus/Pro/Teamに加入済みなら、Codex CLIはそのまま使えます。** 初回起動時に `codex` で認証すればOK。APIキーは不要です。Anthropic（Claude Code）の場合はAPIキーが別途必要です。
...
| リスナーは動くがCodexがエラー | Codex未認証 | `codex` を一度実行してログイン、またはAPIキーを設定 |
